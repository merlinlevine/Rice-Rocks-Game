...
class Sprite:
    ...
    def collide(self, other_object):
        pos_dist = dist(other_object.get_position(), self.pos)
        if (other_object.get_radius() + self.radius) >= pos_dist:
            return True
        else:
            return False
...
def draw(canvas):
    global time, started, a_rock, rock_group, lives, score
...
def rock_spawner():
    ...
    pos_dist = dist(my_ship.get_position(), rock_pos)
    if started == True:
        if (4 * my_ship.get_radius()) < pos_dist:
...
def group_group_collide(group1, group2):
    ...
    for obj in list(group2):
        if group_collide(group1, obj) == True:
            group2.remove(obj)
            collisions += 1
    return collisions
